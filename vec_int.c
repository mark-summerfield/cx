...
vec_int vec_int_alloc_cap(ptrdiff_t cap) {
    cap = cap ? cap : 32;
    VEC_INT_VALUE_T* values = malloc(cap * sizeof(VEC_INT_VALUE_T));
    assert_alloc(values);
    return (vec_int){._count = 0, ._cap = cap, ._values = values};
}
...
inline void vec_int_clear(vec_int* v) { v->_count = 0; }

VEC_INT_VALUE_T vec_int_get(const vec_int* v, ptrdiff_t index) {
    assert_notnull(v);
...
void vec_int_set(vec_int* v, ptrdiff_t index, VEC_INT_VALUE_T value) {
    assert_notnull(v);
    assert_valid_index(v, index);
    v->_values[index] = value;
}

void vec_int_insert(vec_int* v, ptrdiff_t index, VEC_INT_VALUE_T value) {
    assert_notnull(v);
...
void vec_int_add(vec_int* v, VEC_INT_VALUE_T value) {
    ...
    if (!v->_count || v->_values[high] <= value) {
        vec_int_push(v,
...
VEC_INT_VALUE_T vec_int_replace(vec_int* v, ptrdiff_t index,
...
void vec_int_remove(vec_int* v, ptrdiff_t index) {
    assert_notnull(v);
    assert_valid_index(v, index);
...
VEC_INT_VALUE_T vec_int_take(vec_int* v, ptrdiff_t index) {
    assert_notnull(v);
    assert_valid_index(v, index);
    VEC_INT_VALUE_T old = v->_values[index];
...
vec_int vec_int_copy(const vec_int* v) {
    assert_notnull(v);
    vec_int vc = vec_int_alloc_cap(v->_count);
...
ptrdiff_t vec_int_find(const vec_int* v, VEC_INT_VALUE_T value) {
    assert_notnull(v);
    for (ptrdiff_t i = 0; i < v->_count; ++i) {
...
void vec_int_sort(vec_int* v) {
    assert_notnull(v);
    if (v->_count) {
        qsort(v->_values, v->_count, sizeof(VEC_INT_VALUE_T), intcmp);
    }
}

ptrdiff_t vec_int_search(const vec_int* v, VEC_INT_VALUE_T i) {
    assert_notnull(v);
    if (v->_count) {
        const VEC_INT_VALUE_T* p = bsearch(&i, v->_values, v->_count,
...
char* vec_int_tostring(const vec_int* v) {
    assert_notnull(v);
    const ptrdiff_t BUF_SIZE = 128;
    const ptrdiff_t VEC_SIZE = vec_int_count(v);
    ptrdiff_t cap = VEC_SIZE * 4;
    char* s = malloc(cap);
    assert_alloc(s);
    ptrdiff_t pos = 0;
    char buf[BUF_SIZE];
